...
// 因此，k只能小于j,但是它具体在哪里呢？
// 0---(k-1),k---(j-k)---(j-1),j---(i-j)---(i-1),i src
// 		   		     0---(k-1),k---(j-k)---(j-1),j---(i-j)---(i-1),i dest
//				                       0---(k-1),k---(j-k)---(j-1),j---(i-j)---(i-1),i dest1
// 上面的过程中，我们将dest向右滑动，滑动的过程中，我们可以发现
// p[0:k-1] = p[i-k:i-1]的时候，才能停下来比较
// 因为有p[0:j-1] = p[i-j:i-1]，我们可以认为
// p[0:k-1] = p[j-k:j-1]
