...
func findMedianSortedArrays(nums1 []int, nums2 []int) float64 {
	...
	for left <= right {
		...
		nums1SplihtR := math.MaxInt // nums1分割线右边的元素(默认取int最大值)
		...
		if i != m { // 如果nums1有元素划分到右边，则对nums2SplightR重新赋值
			nums1SplihtR = nums1[i]
		}
...
func TestFndMedianSortedArrays(t *testing.T) {
